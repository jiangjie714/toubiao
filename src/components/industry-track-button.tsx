...
import { trackIndustryAction, untrackIndustryAction } from "@/app/actions/industry";
import { RadarIcon, CheckIcon } from "@/components/icons";

export default function IndustryTrackButton({
  industryCode,
  ...
  const handleToggle = async () => {
    ...
    try {
      if (isWatched) {
        const res = await untrackIndustryAction(industryCode);
        if (res.success) {
          setIsWatched(false);
          setMsg("已取消赛道周报");
        } else {
        ...
        const res = await trackIndustryAction(industryCode);
        if (res.success) {
          setIsWatched(true);
          setMsg("已订阅该赛道每周商机推送");
        } else {
          setMsg(res.error || "订阅失败");
        }
      }
    } catch {
      setMsg("网络异常，请重试");
